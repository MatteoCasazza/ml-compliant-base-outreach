...
Outputs
-------
results/optimization_gp_de_v2/gp_de_results.csv
results/optimization_gp_de_v2/gp_de_all_validated_candidates.csv
results/optimization_gp_de_v2/gp_de_convergence.csv
results/optimization_gp_de_v2/gp_de_summary.csv
results/optimization_gp_de_v2/gp_de_objective_settings.csv
results/optimization_gp_de_v2/gp_de_uncertainty_diagnostics.csv

figures/optimization_gp_de_v2/gp_de_target_tracking.png
figures/optimization_gp_de_v2/gp_de_target_error.png
figures/optimization_gp_de_v2/gp_de_constraint_validation.png
figures/optimization_gp_de_v2/gp_de_predicted_vs_true.png
figures/optimization_gp_de_v2/gp_de_optimized_parameters.png
figures/optimization_gp_de_v2/gp_de_convergence.png
figures/optimization_gp_de_v2/gp_de_uncertainty_diagnostics.png
figures/optimization_gp_de_v2/time_response_target*.png
...
RESULTS_DIR = PROJECT_ROOT / "results" / "optimization_gp_de_v2"
FIGURES_DIR = PROJECT_ROOT / "figures" / "optimization_gp_de_v2"
...
class GPDEConfig:
    ...
    # Number of surrogate-ranked candidates validated with the true simulator
    # for each DE attempt. This makes the final selection less dependent on
    # a single polished surrogate optimum.
    ...
    # Optional beta sweep for uncertainty-aware constraint handling.
    # beta controls the conservative constraint value:
    #     max_abs_xr_pred + beta * std_max_abs_xr <= robot_limit_opt
    run_beta_sweep: bool = True
...
def collect_surrogate_ranked_candidates(
    ...
    """
    Collect the best surrogate-ranked candidates from the final DE population.

    Returns a list of (x_vector, surrogate_objective, candidate_rank).
    The polished optimum result.x is always included and ranked together with
    the population candidates.
    """
    candidates: List[np.ndarray] = [np.asarray(result.x, dtype=float)]
    ...
    # Remove near-duplicates to avoid wasting true simulator calls.
    ...
    for x in unique_candidates:
        j = objective_gp_de(x, y_target, models, config)
        scored.append((x, float(j)))

    scored.sort(key=lambda item: item[1])
    top = scored[: max(1, int(top_k))]
    return [(x, j, rank + 1) for rank, (x, j) in enumerate(top)]
...
def build_attempt_record(
    ...
    surrogate_objective_value: Optional[float] = None,
) -> Dict[str, Any]:
    ...
    record = {
        ...
        "objective_value": float(terms["objective_value"] if surrogate_objective_value is None else surrogate_objective_value),
        ...
        "predicted_mean_safety_violation_mm": float(predicted_mean_safety_violation_m * 1000.0),
...
def optimize_target(
    ...
    for attempt_id in range(config.n_optimization_attempts):
        ...
        candidate_records = []
        for x_candidate, surrogate_j, candidate_rank in ranked_candidates:
            record = build_attempt_record(
                ...
                surrogate_objective_value=surrogate_j,
            )
...
def run_beta_sweep(models: Dict[str, Any], base_config: GPDEConfig) -> pd.DataFrame:
    """
    ...
    for beta_value in base_config.beta_sweep_values:
        for sweep_target in base_config.beta_sweep_targets:
            ...
            print(f"\nBeta constraint std = {beta_value:g} | target = {sweep_target:.3f} m")
            best_record, attempts_df, _ = optimize_target(
            ...
            row["sweep_maxiter"] = sweep_config.de_maxiter
            row["sweep_popsize"] = sweep_config.de_popsize
            row["sweep_validated_candidates"] = len(attempts_df)
...
def plot_beta_sweep(sweep_df: pd.DataFrame, config: GPDEConfig) -> None:
    ...
    axes[1].axhline(0.0, linestyle="--", linewidth=2, color="tab:red", label="Constraint boundary")
...
def create_summary_table(results_df: pd.DataFrame, attempts_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # attempts_df contains every true-validated candidate. DE optimization time and
    # surrogate evaluations are identical for candidates from the same attempt, so
    # use unique attempts for those computational-cost metrics.
        ...
        [
            {
                ...
                "mean_residual_margin_mm": float(((0.500 - results_df["max_abs_xr_true"]) * 1000.0).mean()),
                "min_residual_margin_mm": float(((0.500 - results_df["max_abs_xr_true"]) * 1000.0).min()),
                ...
                "total_optimization_time_s_all_attempts": float(unique_attempts_df["optimization_time_s"].sum()),
                "mean_surrogate_function_evaluations_best_attempts": float(results_df["surrogate_function_evaluations"].mean()),
                "total_surrogate_function_evaluations_all_attempts": int(unique_attempts_df["surrogate_function_evaluations"].sum()),
...
def plot_target_error(results_df: pd.DataFrame) -> None:
    ...
    plt.axhline(10.0, linestyle="--", linewidth=2, label="10 mm tolerance")
    plt.xlabel("Target outreach [m]")
...
def plot_constraint_validation(results_df: pd.DataFrame, config: GPDEConfig) -> None:
    ...
    plt.figure(figsize=(8, 6))
    plt.axhspan(config.robot_limit_opt, config.robot_limit_true, alpha=0.15, label="Safety margin band")
...
def plot_predicted_vs_true(results_df: pd.DataFrame) -> None:
    ...
    y_std = results_df["peak_y_std"].to_numpy() if "peak_y_std" in results_df.columns else None
    axes[0].errorbar(
        y_true,
        y_pred,
        yerr=y_std,
    ...
    x_std = results_df["max_abs_xr_std"].to_numpy() if "max_abs_xr_std" in results_df.columns else None
    axes[1].errorbar(
        x_true,
        x_pred,
        yerr=x_std,
...
def plot_residual_margin(results_df: pd.DataFrame, config: GPDEConfig) -> None:
    ...
    residual_margin_mm = (config.robot_limit_true - df["max_abs_xr_true"]) * 1000.0

    plt.figure(figsize=(8, 6))
    plt.bar(df["target"].astype(str), residual_margin_mm, edgecolor="black")
    plt.axhline(0.0, linestyle="--", linewidth=2, label="Constraint boundary")
    plt.axhline(5.0, linestyle=":", linewidth=2, label="5 mm reference margin")
    plt.xlabel("Target outreach [m]")
    plt.ylabel("Residual margin [mm]")
    plt.title("GP + DE residual safety margin")
    plt.grid(True, axis="y", alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def plot_optimized_parameters_physical(results_df: pd.DataFrame, config: GPDEConfig) -> None:
    """Plot optimized parameters in physical units."""
    df = results_df.sort_values("target")
    params = config.optimized_columns()
    ...
    fig.suptitle("Optimized controllable parameters in physical units: GP + DE", fontsize=15, fontweight="bold")
    plt.tight_layout(rect=[0, 0, 1, 0.95])
...
def plot_time_responses(results_df: pd.DataFrame, config: GPDEConfig) -> None:
    ...
    for _, row in results_df.sort_values("target").iterrows():
        ...
        axes[0].axhline(target, linestyle=":", linewidth=2, label=f"Target {target:.3f} m")
        axes[0].axhline(config.robot_limit_true, linestyle="--", linewidth=1.5, label="Nominal robot reach")
        ...
        axes[1].axhline(config.robot_limit_true, linestyle="--", linewidth=2, label="+ robot limit")
        axes[1].axhline(-config.robot_limit_true, linestyle="--", linewidth=2, label="- robot limit")
...
def main() -> None:
    ...
    if config.run_lambda_sweep:
        ...
        print(
            ...
            "gp_de_lambda_sweep.csv before deciding whether to change "
            "lambda_constraint for the final run.\n"
        )
    ...
    attempts_path = RESULTS_DIR / "gp_de_all_validated_candidates.csv"
    ...
    print(f"  {FIGURES_DIR / 'gp_de_residual_margin.png'}")
