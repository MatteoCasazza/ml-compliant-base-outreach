...
Gaussian Process Regression surrogate model for predicting the robot
displacement constraint quantity max_xr.
...
The GP_max_xr model is used as an auxiliary constraint surrogate inside
...
TARGET_COLUMN = "max_xr"

ROBOT_LIMIT_TRUE = 0.50
ROBOT_LIMIT_OPT = 0.495

NEAR_BOUNDARY_LOW = 0.48
NEAR_BOUNDARY_HIGH = 0.52
...
def load_constraint_dataset(
    ...
    """
    Load dataset for the max_xr surrogate.
    ...
    y : ndarray, shape (n_samples,)
        Target vector corresponding to max_xr.
    df : DataFrame
        Full dataset.
    """
    ...
    if TARGET_COLUMN not in df.columns:
        raise ValueError(
            ...
            "The dataset must contain simulation metrics including max_xr."
        )
...
def print_dataset_summary(df: pd.DataFrame) -> None:
    """
    Print dataset and constraint statistics.
    """
    y = df[TARGET_COLUMN].values
    ...
    print("\n" + "=" * 70)
    print("DATASET CONSTRAINT SUMMARY")
    print("=" * 70)

    print(f"Dataset samples:               {len(df)}")
    print(f"Target column:                 {TARGET_COLUMN}")
    print(f"Mean max_xr:                   {np.mean(y):.6f} m")
    print(f"Std max_xr:                    {np.std(y):.6f} m")
    print(f"Min max_xr:                    {np.min(y):.6f} m")
    print(f"Max max_xr:                    {np.max(y):.6f} m")
    ...
    if "constraint_violation" in df.columns:
        print(f"Mean constraint violation:     {df['constraint_violation'].mean() * 1000:.3f} mm")
        print(f"Max constraint violation:      {df['constraint_violation'].max() * 1000:.3f} mm")
...
def train_gp(
    ...
    print("\nFitting GP_max_xr. This may take some time...")
...
def evaluate_classification(
    ...
    """
    Evaluate feasibility classification.
...
def evaluate_near_boundary(
    ...
    """
    Evaluate prediction quality near the robot constraint boundary.

    This is important because GP_max_xr is mainly used to avoid solutions close
    to or above the robot displacement limit.
    """
...
def evaluate_model(
    ...
    print("\n--- CONSTRAINT CLASSIFICATION, OPTIMIZATION SAFETY LIMIT ---")
...
def analyze_test_outliers(
    ...
    """
    Analyze largest test errors for GP_max_xr.
    """
    ...
    error_df["true_max_xr"] = y_test_true
    error_df["pred_max_xr"] = y_test_pred
    ...
    error_df["true_feasible"] = y_test_true <= ROBOT_LIMIT_TRUE
    error_df["pred_feasible"] = y_test_pred <= ROBOT_LIMIT_TRUE
    error_df["true_violation_m"] = np.maximum(0.0, y_test_true - ROBOT_LIMIT_TRUE)
    error_df["pred_violation_m"] = np.maximum(0.0, y_test_pred - ROBOT_LIMIT_TRUE)
    ...
    filepath = save_dir / "test_prediction_errors_max_xr.csv"
    error_df.to_csv(filepath, index=False)

    print("\n" + "=" * 70)
    print("TEST OUTLIER ANALYSIS: GP_MAX_XR")
    print("=" * 70)
    ...
    false_feasible_df = error_df[
        (error_df["pred_feasible"] == True)
        & (error_df["true_feasible"] == False)
    ]

    if len(false_feasible_df) > 0:
        print("\nFalse-feasible test cases:")
        print(false_feasible_df[cols].to_string(index=False))
    else:
        print("\nNo false-feasible test cases.")
...
def analyze_ard_relevance(
    ...
    """
    print("\n" + "=" * 70)
    print("ARD PARAMETER RELEVANCE: GP_MAX_XR")
    print("=" * 70)
    ...
    print("\n✓ Top 3 most influential parameters for max_xr:")
...
def plot_training_results(
    ...
    """
    Generate GP_max_xr diagnostic plots.
    """
    save_dir = Path(save_dir)
    save_dir.mkdir(parents=True, exist_ok=True)
    ...
    ax1.set_xlabel("True max_xr [m]")
    ax1.set_ylabel("Predicted max_xr [m]")
    ...
    ax2.set_xlabel("True max_xr [m]")
    ax2.set_ylabel("Predicted max_xr [m]")
    ax2.set_title(f"Test Set (R² = {metrics['test_r2']:.4f})")
    ax2.legend(fontsize=9)
    ax2.grid(True, alpha=0.3)

    plt.suptitle("Parity Plot: GP_max_xr Constraint Surrogate", fontsize=15)
    plt.tight_layout()

    filepath = save_dir / "max_xr_parity_plot.png"
    ...
    ax1.set_xlabel("Predicted max_xr [m]")
    ...
    ax2.set_xlabel("Predicted max_xr [m]")
    ax2.set_ylabel("Residual [mm]")
    ax2.set_title(f"Test residuals (RMSE = {metrics['test_rmse_mm']:.2f} mm)")
    ax2.legend(fontsize=9)
    ax2.grid(True, alpha=0.3)

    plt.suptitle("Residual Analysis: GP_max_xr", fontsize=15)
    plt.tight_layout()

    filepath = save_dir / "max_xr_residuals.png"
    plt.savefig(filepath, dpi=300, bbox_inches="tight")
    print(f"✓ Saved: {filepath}")
    plt.close()
    ...
    ax.plot(x, y_true_sorted, linewidth=2, label="True max_xr")
    ax.plot(x, y_pred_sorted, linewidth=2, label="Predicted max_xr")
    ...
    ax.set_xlabel("Test sample, sorted by true max_xr")
    ax.set_ylabel("max_xr [m]")
    ax.set_title("GP_max_xr Predictions with Predictive Uncertainty")
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3)

    plt.tight_layout()

    filepath = save_dir / "max_xr_uncertainty.png"
    plt.savefig(filepath, dpi=300, bbox_inches="tight")
    print(f"✓ Saved: {filepath}")
    plt.close()
...
def plot_classification_summary(
    ...
    plt.suptitle("Constraint Classification Confusion Matrices")
    plt.tight_layout()

    filepath = save_dir / "max_xr_constraint_classification.png"
    plt.savefig(filepath, dpi=300, bbox_inches="tight")
    print(f"✓ Saved: {filepath}")
    plt.close()
...
def plot_ard_relevance(
    ...
    ax.set_title("ARD Parameter Relevance: GP_max_xr")
    ...
    filepath = save_dir / "max_xr_ard_relevance.png"
    plt.savefig(filepath, dpi=300, bbox_inches="tight")
    print(f"✓ Saved: {filepath}")
    plt.close()
...
def save_model(
    ...
    """
    Save trained GP_max_xr model, scalers, metrics, ARD and model info.
    """
    save_dir = Path(save_dir)
    save_dir.mkdir(parents=True, exist_ok=True)

    joblib.dump(gp, save_dir / "gp_max_xr_model.pkl")
    print(f"✓ GP_max_xr saved: {save_dir / 'gp_max_xr_model.pkl'}")

    joblib.dump(scaler_X, save_dir / "scaler_X_max_xr.pkl")
    joblib.dump(scaler_y, save_dir / "scaler_y_max_xr.pkl")
    ...
    pd.DataFrame([metrics_clean]).to_csv(
        save_dir / "metrics_max_xr.csv",
        index=False,
    )
    print(f"✓ Metrics saved: {save_dir / 'metrics_max_xr.csv'}")

    ard_df.to_csv(save_dir / "ard_relevance_max_xr.csv", index=False)
    print(f"✓ ARD saved: {save_dir / 'ard_relevance_max_xr.csv'}")

    with open(save_dir / "model_info_max_xr.txt", "w", encoding="utf-8") as f:
        f.write("Gaussian Process Constraint Surrogate Model\n")
        f.write("=" * 50 + "\n")
        f.write("Target: max_xr\n")
        ...
        f.write(f"RMSE: {metrics['test_rmse_m']:.6f} m "
                f"({metrics['test_rmse_mm']:.3f} mm)\n")
        f.write(f"MAE:  {metrics['test_mae_m']:.6f} m "
                f"({metrics['test_mae_mm']:.3f} mm)\n")
        f.write("\nConstraint classification, true limit:\n")
        f.write(
            f"Accuracy: "
        ...
        f.write(
            f"False infeasible: "
            f"{metrics['true_limit_false_infeasible_rate'] * 100:.2f}%\n"
        )
        f.write("\nNear-boundary metrics:\n")
        f.write(
            f"Range: [{NEAR_BOUNDARY_LOW:.3f}, {NEAR_BOUNDARY_HIGH:.3f}] m\n"
        )
        f.write(
            f"Samples: {metrics['near_boundary_n_samples']}\n"
        )
        f.write(
            f"RMSE: {metrics['near_boundary_rmse_mm']:.3f} mm\n"
        )
        f.write(
            f"False feasible: "
    ...
    print(f"✓ Model info saved: {save_dir / 'model_info_max_xr.txt'}")
...
def load_model_max_xr(
    ...
    """
    Load trained GP_max_xr model and scalers.
    """
    save_dir = Path(save_dir)

    gp = joblib.load(save_dir / "gp_max_xr_model.pkl")
    scaler_X = joblib.load(save_dir / "scaler_X_max_xr.pkl")
    scaler_y = joblib.load(save_dir / "scaler_y_max_xr.pkl")

    print(f"✓ GP_max_xr loaded from: {save_dir}")
...
def save_test_predictions(
    ...
    """
    Save detailed test predictions for GP_max_xr.
    """
    save_dir = Path(save_dir)
    save_dir.mkdir(parents=True, exist_ok=True)
    ...
    predictions_df["true_max_xr"] = y_test_true
    predictions_df["pred_max_xr"] = y_test_pred
    ...
    predictions_df["true_feasible_true_limit"] = y_test_true <= ROBOT_LIMIT_TRUE
    predictions_df["pred_feasible_true_limit"] = y_test_pred <= ROBOT_LIMIT_TRUE

    predictions_df["true_feasible_opt_limit"] = y_test_true <= ROBOT_LIMIT_OPT
    predictions_df["pred_feasible_opt_limit"] = y_test_pred <= ROBOT_LIMIT_OPT

    predictions_df["true_violation_m"] = np.maximum(0.0, y_test_true - ROBOT_LIMIT_TRUE)
    predictions_df["pred_violation_m"] = np.maximum(0.0, y_test_pred - ROBOT_LIMIT_TRUE)
    ...
    path = save_dir / "test_predictions_max_xr.csv"
...
def main() -> None:
    """
    Run the GP_max_xr training pipeline.
    """
    print("\n" + "=" * 70)
    print("GAUSSIAN PROCESS REGRESSION - GP_MAX_XR TRAINING PIPELINE")
    print("=" * 70 + "\n")
    ...
    # Final summary
    print("\n" + "=" * 70)
    print("GP_MAX_XR TRAINING COMPLETED")
    print("=" * 70)
    ...
    print("\nTOP 3 INFLUENTIAL PARAMETERS FOR max_xr:")
    ...
    print("\nOUTPUT:")
    print(f"  Model:             {GP_CONSTRAINT_RESULTS_DIR / 'gp_max_xr_model.pkl'}")
    print(f"  Scalers:           {GP_CONSTRAINT_RESULTS_DIR / 'scaler_*_max_xr.pkl'}")
    print(f"  Metrics:           {GP_CONSTRAINT_RESULTS_DIR / 'metrics_max_xr.csv'}")
    print(f"  ARD:               {GP_CONSTRAINT_RESULTS_DIR / 'ard_relevance_max_xr.csv'}")
    print(f"  Model info:         {GP_CONSTRAINT_RESULTS_DIR / 'model_info_max_xr.txt'}")
    print(f"  Predictions:        {GP_CONSTRAINT_RESULTS_DIR / 'test_predictions_max_xr.csv'}")
    print(f"  Error analysis:     {GP_CONSTRAINT_RESULTS_DIR / 'test_prediction_errors_max_xr.csv'}")
    print(f"  Plots:              {GP_CONSTRAINT_FIGURES_DIR / 'max_xr_*.png'}")
