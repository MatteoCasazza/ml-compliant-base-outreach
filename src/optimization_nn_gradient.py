...
The optimization loop uses the trained deterministic NN surrogate:

    NN_multioutput : predicts [peak_y, max_abs_xr]
...
-----------------------------
The NN is deterministic and does not provide predictive uncertainty. Therefore
its optimization objective uses the same tracking and constraint terms as GP+DE,
but no GP-style uncertainty penalty. Absolute objective values should not be
compared directly between GP+DE and NN+Adam; compare only final physical metrics
after true simulation validation.
...
class NNGradientConfig:
    ...
    # ------------------------------------------------------------------
    run_learning_rate_sweep: bool = True
...
def nn_predict_from_xopt(
    ...
    Y_scaled = models["model"](X_scaled)
